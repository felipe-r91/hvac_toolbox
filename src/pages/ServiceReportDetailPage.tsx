...
import { type ServiceReportDraft } from "../types/maintenance";

type Props = {
  serviceReportDrafts: ServiceReportDraft[];
};

function formatReturnedToService(value: ServiceReportDraft["machineReturnedToService"]) {
  switch (value) {
    case "yes":
      return "Yes";
    case "no":
...
export function ServiceReportDetailPage({ serviceReportDrafts }: Props) {
  const { draftId } = useParams();

  const draft = serviceReportDrafts.find((item) => item.id === draftId);

  if (!draft) {
    return <div className="p-6">Service report not found.</div>;
  }
    ...
    <section className="space-y-4">
      ...
      <section className="rounded-3xl bg-white p-5 shadow-sm ring-1 ring-slate-200">
        ...
        <div className="mt-4 space-y-4">
          <DetailBlock title="Work Performed" value={draft.workPerformed} />
